...
router.post("/Join", function(request, response){
    ...
    let sql = "insert into message_member values(?,?,?,?,now())";               //오라클은 sysdate  ===> mysql은 now()
    conn.query(sql, [id, pw, tel, address], function(err, rows){
...
router.post("/Login", function(request, response){
    ...
    let sql = "select * from message_member where id=? and pw=?";
...
router.post("/update_exe", function(request, response){
    ...
    sql = "update message_member set pw = ?, tel = ?, address = ? where id = ?";
    conn.query(sql, [pw, tel, address, id], function(err, rows){
        if(rows){
            request.session.user = {
                "id" : id,
                "tel" : tel,
